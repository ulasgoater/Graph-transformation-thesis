...
def load_autograph_csv(path):
    """
    Load autograph CSV and build LineString geometries from geometry/{i}/lat & geometry/{i}/lon columns.
    Returns GeoDataFrame in WORK_CRS (EPSG:3857).
    """
    ...
    if not geom_idx:
        raise RuntimeError("Autograph CSV: no geometry/*/lat columns detected.")
...
def load_point_csv(path, lon_candidates=("lon","longitude"), lat_candidates=("lat","latitude","center_lon","center/long","center/lat","center_lon")):
    ...
    if lon_col is None or lat_col is None:
        raise RuntimeError(f"No lon/lat columns found in {path}")
